...
gradientTheme.addEventListener("click", () => {
  ...
  document.querySelector("header").classList.toggle("gradient-mode");
  document.querySelector("footer").classList.toggle("gradient-mode");
});

neonTheme.addEventListener("click", () => {
  document.body.classList.toggle("neon-mode");
  document.querySelector("header").classList.toggle("neon-mode");
  document.querySelector("footer").classList.toggle("neon-mode");
});

woodenTheme.addEventListener("click", () => {
  document.body.classList.toggle("wooden-mode");
  document.querySelector("header").classList.toggle("wooden-mode");
  document.querySelector("footer").classList.toggle("wooden-mode");
});
...
// Audio Elements (Fallbacks added so no console errors occur if missing)
...
// Robust Sleep function that handles real-time pausing natively
function sleep(ms) {
...
function startTimer() {
  ...
  timerInterval = setInterval(() => {
    if (!isPaused && isSorting) {
      timeDisplay.textContent = Math.floor(performance.now() - startTime);
    }
  }, 50);
}
...
// Update the bottom card metrics block
function updateComparisonSection(algo) {
  const algoMetricPrefix = algo.replace("Sort", ""); // turns 'bubbleSort' into 'bubble'
  const compEl = document.getElementById(`${algoMetricPrefix}Comparisons`);
  const swapEl = document.getElementById(`${algoMetricPrefix}Swaps`);

  if (compEl) compEl.textContent = comparisons;
  if (swapEl) swapEl.textContent = swaps;
}

// Master Run Control Trigger Function
async function startSortingProcess() {
  ...
  stopTimer();
  updateComparisonSection(algorithm);
  isSorting = false;
}
...
sortButton.addEventListener("click", startSortingProcess);
...
// NEW: Wire up the "Algorithm Tutorials" sections
document.querySelectorAll(".tutorialButton").forEach((btn) => {
  btn.addEventListener("click", (e) => {
    const targetAlgo = e.target.getAttribute("data-algorithm");
    
    // 1. Change drop down select value
    algorithmSelect.value = targetAlgo;
    algorithmSelect.dispatchEvent(new Event("change"));
    
    // 2. Scroll smooth up to visualizer container
    document.getElementById("visualizer").scrollIntoView({ behavior: "smooth" });
    
    // 3. Kick off structural run
    reset();
    startSortingProcess();
  });
});
