...
// =====================================================================
// 2. INSTANT AUDIO SYNCHRONIZATION BACKEND
// =====================================================================
const swapSound = new Audio("swap.wav");
const compareSound = new Audio("compare.wav");

// Clones the track and forces an immediate zero-timestamp play to prevent overlapping delays
function playSoundInstant(audioNode) {
  if (!audioNode) return;
  try {
    const soundClone = audioNode.cloneNode();
    soundClone.currentTime = 0;
    soundClone.volume = 0.3; // Slight drop to keep rapid overlapping clean
    soundClone.play().catch(() => {});
  } catch (e) {
    // Graceful fallback if audio is blocked or missing
  }
}

// =====================================================================
// 3. REUSABLE SYSTEM CORE COMPONENT: RENDERING GRAPHICS
// =====================================================================
...
// =====================================================================
// 4. CORE ENGINE SWAP HANDLER (SHARED PIPELINE)
// =====================================================================
async function sharedSwap(engineState, i, j) {
  ...
  if (engineState.isMainEngine) {
    playSoundInstant(swapSound);
  }
  await sleep(delay);
}

// =====================================================================
// 5. MASTER SORT PIPELINES WITH AUDIO SNAPPING
// =====================================================================
async function runBubbleSort(state) {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    for (let j = 0; j < arr.length - i - 1; j++) {
      ...
      if (state.isMainEngine) playSoundInstant(compareSound);
      await sleep(delay);
...
async function runInsertionSort(state) {
  ...
  for (let i = 1; i < arr.length; i++) {
    ...
    if (state.isMainEngine) playSoundInstant(swapSound);
    await sleep(delay);
  }
}
...
// =====================================================================
// 6. PARALLEL DUAL COMPARISON HANDLER
// =====================================================================
if (startComparisonBtn) {
  startComparisonBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      console.error("Parallel comparison pipeline engine execution error:", err);
    } finally {
      isComparing = false;
...
// =====================================================================
// 7. MAIN INTERFACE EVENT CONTROLS
// =====================================================================
...
try {
  ...
  console.warn("Primary container context target skipped during window assembly.", e);
}
